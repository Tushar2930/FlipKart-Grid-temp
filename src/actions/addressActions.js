...
export const getAddresses = () => async (dispatch, getState) => {
  ...
  if (isAuthenticate) {
    try {
      const { data } = await axios.get(`https://flipkart-web.vercel.app/api/address/get-addresses/${user._id}`);
      dispatch({
...
export const deleteAddress = (id) => async (dispatch) => {
    try {
      await axios.delete("https://flipkart-web.vercel.app/api/address/delete-address", {
        data: {
          addressId: id,
        },
      });
