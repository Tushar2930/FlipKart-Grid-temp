...
export const getProductById = (id) => async (dispatch) => {
  try {
    const { data } = await axios.get(`https://flipkart-web.vercel.app/api/products/get-product/${id}`);
    dispatch({
...
export const getProductsByCategory = async(name) => {
  try {
    const { data } = await axios.get(`https://flipkart-web.vercel.app/api/products/get-products/${name}`);
    return data;
  } catch (error) {
    console.log(error);
    return null;
